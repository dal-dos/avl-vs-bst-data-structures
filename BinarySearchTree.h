...
private:
    ...
    int getSize(BinaryNode* t) const
    {
        if (t != nullptr)
        {
    ...
    int getHeight(BinaryNode* t, int k) const
    {
        if (t != nullptr)
        {
            return max(getHeight(t->left, (k + 1)) , getHeight(t->right, (k + 1))) + 1;
        }
        return -1;
    }

    int ipl(BinaryNode* t,int k) const
    {
        if (t != nullptr)
        {
            return ipl(t->left, (k + 1)) + ipl(t->right, (k + 1)) + k;
        }
        return 0;
    }
